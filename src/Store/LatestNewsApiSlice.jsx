  ...
  endpoints: (builder) => ({
    getTasks: builder.query({
      query: ({ page, lang, country, Category, Priority }) => {
        const k1=import.meta.env.VITE_API_URL1;
    ...
    getTopHeading: builder.query({
      query: ({lang,country}) => ({
        url: `https://newsapi.org/v2/top-headlines?country=${country}&apiKey=${import.meta.env.VITE_API_URL2}&pageSize=100&language=${lang}`,
        method: "GET",
      }),
    }),
    getEveryNews:builder.query({
      query:({lang,page,search})=>({
        url:`https://newsapi.org/v2/everything?q=${search ? search :'a'}&apiKey=${import.meta.env.VITE_API_URL2}&pageSize=50&languge=${lang}&page=${page}`,
        method:'GET'
      })
    })
  }),
